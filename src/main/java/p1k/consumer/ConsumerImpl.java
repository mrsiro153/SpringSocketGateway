...
import vn.ntex.bank.repository.common.constant.QueueKeys;
...
public class ConsumerImpl implements IConsumer {
    ...
    public void handleMessage(QueueKeys.Socket_Queue Queue) {
        try {
            ...
            LOGGER.info("handle Message from queue: {}",Queue.getValue());
                    ...
                    threadPoolTaskExecutor.execute(() -> {//open new thread
                        switch (Queue) {
                            ...
                            case DIRECT_SOCKET_WEB_QUEUE:
                                socketService.handleSendMessageDirectToFrontEnd(message);
                                break;
                            default:
